  ...
  <v-app>
    ...
    <v-main style="background-color: #efe5fd">
                              ...
                              <v-tab-item>
                                ...
                                >
                                  <div class="me3 pa-6 text-left">
                                    <v-card-text
                                      v-html="
                                        $md.render(question.questiondescription)
                                      "
                          ...
                          >
                            <v-tabs color="#7776AC" v-model="tab">
                              ...
                              <v-tab-item>
                                <testssolution
                                  :someData="question"
                                  v-on:ChangeTestSolution="
                                    updateTestSolution($event)
                                  "
                                ></testssolution>
                              </v-tab-item>
                              <v-tab-item>
                                <exampletestcases
                                  :someData="question"
                                  v-on:ChangeExampleTestCase="
                                    updateExampleTestCase($event)
                                  "
                                ></exampletestcases>
                              </v-tab-item>
                              <v-tab-item
                                ><test-cases
                                  :someData="question"
                                  v-on:ChangeTestCase="updateTestCase($event)"
                                ></test-cases
                              ></v-tab-item>
                              <v-tab-item
                                ><initsolution
                                  :someData="question"
                                  v-on:ChangeInitSolution="
                                    updateInitSolution($event)
                                  "
                                ></initsolution
                              ></v-tab-item>

                              <v-btn
                                @click="validatetestcase"
                                elevation="2"
                                outlined
                                color="#7776AC"
                                >Validate TestCase</v-btn
                              >
                            </v-tabs>

                            <br /><br /><br />
                            <v-card
                              class="pa-2 rounded-xl mx-auto"
                            ...
                            >
                              ...
                              <div
                                style="color: red"
                                v-if="validateresultrealtest.code == 500"
                              >
                                {{ validateresultrealtest.reason }}
                              </div>
                              ...
                              <div style="padding: 25px">
                                ...
                                <ul>
                                  <li
                                    v-for="item in validateresultrealtest.successes"
                                    :key="item.index"
                                  >
                                    <h3 style="color: green">
                                      {{ item.case }} , Score: {{ item.score }}
                                    </h3>
                                  </li>
                                </ul>
                                ...
                                <ul>
                                  <li
                                    v-for="item in validateresultrealtest.failures"
                                    :key="item.index"
                                  >
                                    <h3 style="color: red">
                                      {{ item.case }} <br />
      ...
      <h3 style="color:green">Test1  (Pass) </h3> -->
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="amber darken-1" text @click="close">
                      Cancel
                    </v-btn>
                    <v-btn
                      color="deep-purple accent-1"
                      text
                      :disabled="
                        validateresultrealtest.code == 500 ||
                        validateresultexampletest.code == 500 ||
                        validateresultrealtest.failures[0] != null ||
                        !valid ||
                        validateresultexampletest.failures[0] != null ||
                        test.testcases == '' ||
              ...
              <v-dialog v-model="dialogDelete" max-width="500px">
                <v-card>
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="amber darken-1" text @click="closeDelete"
                      >Cancel</v-btn
                    >
                    <v-btn
                      color="deep-purple accent-1"
          ...
          <template v-slot:expanded-item="{ headers, item }">
            <td :colspan="headers.length">TestSolution : {{ item.tests }}</td>
          </template>
  ...
  methods: {
    multipledelete() {
      if (this.selected[0] == null) {
        ...
        if (result == true) {
          this.$store
            .dispatch("question/multipledeletequestion", this.selected)
    ...
    validatetestcase() {
      if (this.test.testcase != "" && this.test.exampletestcases != "")
        this.valid = true;
      ...
      this.$store
        ...
        .then((resp) =>
          (this.validateresultrealtest = JSON.parse(resp.data)).catch((err) =>
            console.log(err)
          )
        );
      this.$store
        ...
        .then((resp) =>
          (this.validateresultexampletest = JSON.parse(resp.data)).catch(
            (err) => console.log(err)
          )
        );
    },
    ...
    deleteItemConfirm() {
      ...
      this.$store
        .dispatch("question/deleteonequestion", this.editedQuestion.questionid)
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        this.$store
          .dispatch("question/updatequestion", data)
          .then((resp) => this.fetchnextpage());
